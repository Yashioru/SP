  ...
  return (
    <div style={{ background: "#071a1c", color: "white", fontFamily: "'Syne', sans-serif", overflowX: "hidden" }}>
      <style>{`
        ...
        .pbtn-p{background:#E20074;color:white;box-shadow:0 0 24px rgba(226,0,116,0.5),0 0 48px rgba(226,0,116,0.2)}
        .pbtn-p:hover{background:transparent;color:#E20074;box-shadow:0 0 32px rgba(226,0,116,0.7),0 0 64px rgba(226,0,116,0.3)}
        ...
        .tcard:hover{border-color:#237b82;background:rgba(35,123,130,0.28)}
        input,textarea{background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.15);border-radius:12px;color:white;padding:16px;width:100%;font-family:'Syne',sans-serif;font-size:15px;outline:none;transition:border-color 0.2s;box-sizing:border-box}
      ...
      <header style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, padding:"16px 20px", background: scrolled?"rgba(7,26,28,0.95)":"transparent", backdropFilter:scrolled?"blur(12px)":"none", borderBottom:scrolled?"1px solid rgba(226,0,116,0.2)":"1px solid transparent", transition:"all 0.3s", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <SelfLogo size={36} />
          <span style={{ fontWeight:800, fontSize:18, letterSpacing:"0.08em", textShadow:"0 0 16px rgba(255,255,255,0.2)" }}>SELF<span style={{ color:"#E20074" }}>.</span></span>
        </div>
      ...
      <section id="lineup" style={{ padding:"80px 20px" }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
            ...
            <span className="mono" style={{ fontSize:10, color:"#E20074", letterSpacing:"0.25em", textTransform:"uppercase", textShadow:"0 0 12px rgba(226,0,116,0.8)" }}>Artists</span>
          </div>
          <h2 style={{ fontSize:40, fontWeight:800, marginBottom:8, letterSpacing:"-0.02em", textShadow:"0 0 20px rgba(255,255,255,0.1)" }}>Lineup</h2>
          <div style={{ marginBottom:36 }}><Waveform color="white" opacity={0.2} /></div>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {[
              {name:"DAVID GUETTA",time:"22:00",stage:"Main Stage",headliner:true},
              {name:"MARTIN GARRIX",time:"20:00",stage:"Main Stage",headliner:true},
              {name:"CHAINSMOKERS",time:"18:00",stage:"Stage B",headliner:false},
              {name:"MARSHMELLO",time:"16:00",stage:"Stage B",headliner:false},
              {name:"TBA",time:"14:00",stage:"Stage C",headliner:false},
            ].map(({name,time,stage,headliner})=>(
              <div key={name} className={headliner?"acard":"tcard"}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    <div style={{ fontSize:headliner?18:15, fontWeight:headliner?800:600, color:headliner?"white":"rgba(255,255,255,0.85)", textShadow:headliner?"0 0 20px rgba(255,255,255,0.2)":"none", letterSpacing:"0.03em" }}>{name}</div>
                    <div className="mono" style={{ fontSize:11, color:headliner?"#E20074":"#237b82", marginTop:4, letterSpacing:"0.1em" }}>{stage}</div>
                  </div>
                  <div className="mono" style={{ fontSize:14, color:"rgba(255,255,255,0.5)" }}>{time}</div>
                </div>
              </div>
            ))}
          </div>
          <p style={{ marginTop:20, fontSize:12, color:"rgba(255,255,255,0.4)", fontStyle:"italic" }}>* Full lineup announcement coming soon</p>
        </div>
      </section>

      {/* SCHEDULE */}
      <section id="schedule" style={{ padding:"80px 20px", background:"rgba(35,123,130,0.38)", borderTop:"1px solid rgba(35,123,130,0.3)", borderBottom:"1px solid rgba(35,123,130,0.3)" }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
          ...
          {[
            ...
            <div key={day} style={{ marginBottom:32 }}>
              <div style={{ display:"flex", alignItems:"baseline", gap:16, marginBottom:16 }}>
                <span style={{ fontSize:22, fontWeight:800, color:"#E20074", textShadow:"0 0 16px rgba(226,0,116,0.6)" }}>{day}</span>
                <span className="mono" style={{ fontSize:12, color:"rgba(255,255,255,0.4)" }}>{date}</span>
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:8, paddingLeft:16, borderLeft:"2px solid rgba(226,0,116,0.4)" }}>
                {events.map(e=><div key={e} className="mono" style={{ fontSize:13, color:"rgba(255,255,255,0.85)" }}>{e}</div>)}
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="venue" style={{ padding:"80px 20px" }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
            <div style={{ width:16, height:1, background:"#E20074" }} />
            <span className="mono" style={{ fontSize:10, color:"#E20074", letterSpacing:"0.25em", textTransform:"uppercase" }}>Location</span>
          </div>
      ...
      <section id="tickets" style={{ padding:"80px 20px", background:"rgba(226,0,116,0.07)", borderTop:"1px solid rgba(226,0,116,0.2)" }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          ...
          <h2 style={{ fontSize:40, fontWeight:800, marginBottom:8, letterSpacing:"-0.02em", textShadow:"0 0 20px rgba(255,255,255,0.1)" }}>Tickets</h2>
          <p style={{ color:"rgba(255,255,255,0.78)", fontSize:14, marginBottom:36, lineHeight:1.7 }}>Early bird pricing available for students. Bring your student ID.</p>
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            {[
              {tier:"Early Bird",price:"€35",perks:["1-Day Access","Student Discount","General Area"],hot:false},
              {tier:"Full Pass",price:"€120",perks:["2-Day Access","All Stages","Exclusive Merch"],hot:true},
              {tier:"VIP",price:"€600",perks:["2-Day Access","VIP Area","Meet & Greet"],hot:false},
              ...
              <div key={tier} style={{ border:`1px solid ${hot?"#E20074":"rgba(255,255,255,0.1)"}`, borderRadius:20, padding:"24px", background:hot?"rgba(226,0,116,0.08)":"rgba(255,255,255,0.02)", position:"relative" }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
                  <div style={{ fontSize:18, fontWeight:800, textShadow:"0 0 16px rgba(255,255,255,0.15)" }}>{tier}</div>
                  <div className="mono" style={{ fontSize:20, fontWeight:700, color:hot?"#E20074":"rgba(255,255,255,0.7)", textShadow:hot?"0 0 20px rgba(226,0,116,0.6)":"none" }}>{price}</div>
                </div>
                <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:20 }}>
                  {perks.map(p=>(
                    <div key={p} style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:"rgba(255,255,255,0.82)" }}>
      ...
      <section id="contact" style={{ padding:"80px 20px" }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          ...
          <h2 style={{ fontSize:40, fontWeight:800, marginBottom:8, letterSpacing:"-0.02em", textShadow:"0 0 20px rgba(255,255,255,0.1)" }}>Get In Touch</h2>
          <p style={{ color:"rgba(255,255,255,0.78)", fontSize:14, marginBottom:36 }}>Questions? Sponsorships? Artist bookings? We'd love to hear from you.</p>
        ...
        <div style={{ maxWidth:480, margin:"0 auto", display:"flex", flexDirection:"column", alignItems:"center", gap:20 }}>
          ...
          <div style={{ display:"flex", gap:24 }}>
            {["Instagram","TikTok","Spotify"].map(s=>(
              <a key={s} href="#" className="mono" style={{ fontSize:11, color:"rgba(255,255,255,0.5)", textDecoration:"none", letterSpacing:"0.1em", textTransform:"uppercase", transition:"color 0.2s" }}
                onMouseEnter={e=>(e.currentTarget.style.color="#E20074")}
          ...
          <p className="mono" style={{ fontSize:10, color:"rgba(255,255,255,0.3)", letterSpacing:"0.1em", textAlign:"center" }}>
